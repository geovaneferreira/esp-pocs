...
char ack[4] = {0x1a,0x01,0x79,0x5d};
char start_ota_cmd[4] = {0x5a,0x55,0x5a,0x55};
...
int main(int argc, char* argv[])
{
  ...
  printf("iniciando a serial\n");
  set_interface_attribs (fd, B115200, 0);  // set speed to 115,200 bps, 8n1 (no parity)
  set_blocking (fd, 1);                // set blocking
  printf("serial ok\n");

  printf("enviando bytes de sync\n");
  //write to serial terminal - sync bytes
  write (fd, start_ota_cmd, sizeof(start_ota_cmd));
...
//     printf("\n");
    ...
    {
      printf("Wrote first chunk, waiting 20s for ESP32 to opening new partition\r\n");
      usleep(20000000);
      wait = 1;
    }
